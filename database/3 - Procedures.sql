...
BEGIN
	...
	SELECT
		...
		ROW_NUMBER() OVER(ORDER BY QUANTIDADE_CRITERIOS DESC, NOME ASC),
	...
	SELECT
		@@IDENTITY,
		ID_CANDIDATO_SORTEIO,
		ROW_NUMBER() OVER(ORDER BY NOME ASC),
		1
	FROM
		NOSSACASA.CANDIDATO_SORTEIO
	WHERE
	...
	SELECT
		...
		ID_CANDIDATO_SORTEIO,
		ROW_NUMBER() OVER(
			ORDER BY (CASE WHEN QUANTIDADE_CRITERIOS = 4 THEN 3 ELSE QUANTIDADE_CRITERIOS END) DESC, NOME ASC
		),
		DENSE_RANK() OVER(
			ORDER BY (CASE WHEN QUANTIDADE_CRITERIOS = 4 THEN 3 ELSE QUANTIDADE_CRITERIOS END) DESC
		)
	FROM
		NOSSACASA.CANDIDATO_SORTEIO
	WHERE
